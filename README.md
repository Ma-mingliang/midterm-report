...
| 文件 | 必备 | 作用 | 格式要求 | 命名规范 |
|------|:----:|------|---------|---------|
| **模板.docx** | 是 | 提供 Word 文档的整体格式框架：封面页信息（姓名、学号、导师、专业等）、表格结构（6行3列，奇数行=章节标题，偶数行=章节内容）、字体排版规范 | .docx 格式，必须包含至少1个表格，表格至少6行 | 文件名可任意，如 `李耀威-模板.docx`、`模板.docx` 等，在脚本的 `TEMPLATE_FILE` 变量中指定 |
| **研究生成绩单.pdf** | 是 | 提供课程成绩、总学分、绩点等数据，用于第一章"思想品德与业务学习情况自述"中的成绩描述 | .pdf 格式，通常为学校教务系统导出的标准成绩单 | 文件名可任意，在脚本中作为数据来源引用 |
| **论文.pdf** | 是 | 提供研究的技术细节：方法描述、公式推导、实验数据、图表说明。是第二章技术内容的主要来源，也是图片含义交叉验证的权威依据 | .pdf 格式，为已发表或在投的学术论文 | 文件名可任意，如 `Robust Muscle Synergy...pdf` |
| **中期ppt.pdf** | 是 | 提供完整的工作框架和模块结构。PPT 包含全部已完成工作的概览（论文通常只覆盖部分模块），是识别研究模块边界和图片分配的核心依据 | .pdf 格式（由 PPT 另存为 PDF 导出） | 文件名可任意，如 `中期ppt.pdf`、`中期汇报.pdf` 等 |
| **图片/** | 是 | 存放所有需要插入报告的研究图片。**此目录是报告中图片的唯一来源**，不从 PPT 或论文中提取图片 | 支持 .png/.jpg/.jpeg/.bmp/.gif/.webp/.tiff 格式 | 见下方"图片目录命名规范" |
| **rewrite_report.py** | 是 | 主程序脚本，包含模块定义、公式定义、章节写入函数和格式控制逻辑 | .py 格式，Python 3.10+ | 固定文件名 `rewrite_report.py` |
...
| **modules_output.txt** | PPT 模块检测结果 | 自动生成的模块列表，用于确认模块结构 |
| **SKILL_github.md** | Claude Code 技能文档 | 供 AI 助手使用的详细工作流程规范 |
...
```
图片/
├── 整体框架.png                    # 研究整体框架图（用于报告第1张图）
├── 图01_多模态数据采集装置.jpeg      # 模块1相关图片
├── 图02_数据预处理.jpeg             # 模块1相关图片
├── 图03_协同模式提取.png            # 模块1相关图片
├── 图04_蒸馏架构与ResNet_Mamba.jpeg # 模块2相关图片
├── 图05_最佳微调比例探寻.png        # 模块2相关图片
├── 图06_迁移微调策略.jpeg           # 模块2相关图片
├── 图07_模型性能对比.png            # 模块2相关图片
├── 图08_协同时序重建效能.png        # 模块2相关图片
├── 图09_融合闭环控制律.png          # 模块3/4相关图片
├── 图10_康复系统框图.jpeg           # 模块5相关图片
├── 图11_实验平台.jpeg               # 模块5相关图片
└── 图12_控制效能评估.png            # 模块5相关图片
```

#### 命名规则

1. **整体框架图**：命名为 `整体框架.xxx`，用于报告开篇的第 1 张图（"如图1所示为本文的整体研究框架"）
2. **模块图片**：使用 `图XX_简要描述.xxx` 格式，其中：
   - `XX` 为两位数字编号（01、02、03...），按模块顺序排列
   - `_` 下划线连接编号和描述
   - 描述应简明扼要，体现图片内容（如 `数据采集装置`、`蒸馏架构`）
3. **文件扩展名**：支持 `.png`、`.jpg`、`.jpeg`、`.bmp`、`.gif`、`.webp`、`.tiff`
4. **描述中的特殊字符**：避免使用 `/`、`\`、`:` 等文件系统保留字符，用 `_` 替代
...
| 模块 | 典型图片数 | 示例文件名 |
|------|:---------:|-----------|
| （1）多模态数据采集与预处理 | 2-3 张 | `图01_采集装置.jpeg`、`图02_预处理流程.jpeg`、`图03_VAF分析.png` |
| （2）跨模态泛化协同建模 | 4-6 张 | `图04_蒸馏架构.jpeg`、`图05_微调比例.png`、`图06_迁移策略.jpeg`、`图07_性能对比.png`、`图08_时序重建.png` |
| （3）实时协同缺损评估 | 0-1 张 | `图09_闭环控制律.png` |
| （4）基于MFAC的融合控制律 | 0-1 张 | （与模块3共用同一张图，或无图） |
| （5）实验平台搭建 | 2-3 张 | `图10_系统框图.jpeg`、`图11_实验平台.jpeg`、`图12_效能评估.png` |
| （6）学术成果 | 0 张 | （纯文字描述，无图片） |

**重要约束：**
- 每张图片在报告中**只能使用 1 次**，不允许同一张图出现在多个位置
- 图片目录中的**每张图片都必须在报告中被使用**，不允许遗漏
- 图片总数 = 报告中的图片总数（不多不少）
