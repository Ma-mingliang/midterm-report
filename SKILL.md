---
name: midterm-report
description: 研究生中期考核报告自动生成技能。基于模板文档、PPT、论文PDF和成绩单，自动生成符合学校格式要求的中期考核报告Word文档。支持中英混合字体、OMML公式、图片来源追溯与交叉验证、三层行文逻辑结构，输出≥6000字、15-20张研究图片的完整报告。
---
...
1. **图片来源多样**：用户可能直接提供图片，也可能要求从PPT或论文中提取。图片可能存放在 `图片/` 目录中，也可能需要从源文件中提取。
2. **论文和PPT是核心参考**：用户会提供论文PDF和中期PPT，这些是写作内容的主要来源。需要从中提取技术细节、实验数据、方法描述等。
3. **从源文件中提取图片**：需要从PPT中提取**嵌入式图片**（不仅仅是整页渲染），从论文中提取图表。PPT整页渲染仅用于理解页面结构，嵌入式图片才是实际使用的图表。
4. **必须理解图片的真实含义**：不能仅凭文件名或位置猜测图片用途。必须通过阅读PPT页面文字、论文上下文来确认每张图片**实际表达的内容**，并与图注严格对应。必须交叉验证，确保正确无误。

---

## 模板选择

用户可选择两种行文与排版模板，生成报告前必须确认使用哪一种。

### lyw-模板（工程实现型）
- **文件**：`templates/lyw-模板.md`
...
1. **读取约束文件**：检查工作目录下是否存在 `需求与约束.md`，如不存在则根据以下模板生成
2. **扫描材料清单**：
   - `模板.docx` 或 `XXX-模板.docx` — 格式参考（封面页、表格结构、字体排版）
   - `中期考核相关资料.docx` — 内容要求（考核内容、评分标准）
   - `研究生成绩单.pdf` — 课程成绩、学分、绩点
   - 论文PDF — 技术内容来源
   - `中期ppt.pdf` — 完整工作框架
   - `图片/` 目录 — 用户提供的图片（如有）
3. **确认信息来源**：向用户确认各材料的用途和对应关系
...
├── 模板.docx              # 格式模板（只读）
├── XXX-模板.docx          # 实际使用的模板
├── 中期考核相关资料.docx   # 考核要求（只读）
├── 研究生成绩单.pdf        # 成绩数据（只读）
├── 论文.pdf               # 技术内容来源（只读）
├── 中期ppt.pdf            # 工作框架来源（只读）
├── 图片/                   # 所有待插入报告的图片
├── ppt_images/             # 从PPT提取的嵌入式图片（用于比对和理解）
├── paper_images/           # 从论文提取的图片（用于比对和理解）
├── omml_formulas.py        # OMML公式构建模块
...
3. **不可靠**：仅凭文件名判断图片内容
4. **不可靠**：PPT整页渲染图（可能包含多个元素，不能直接用于报告）
